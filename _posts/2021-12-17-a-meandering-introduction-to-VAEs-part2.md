...
with t.no_grad():
  ...
  x = Bernoulli(probs=p).sample() #Step 3. x ~ p(x | z, 𝜃). shape: (10, 784) 
...
# Computing $p(x|\theta)$ or $\log p(x|\theta)$

So it all boils down to computing $p(x|\theta)$ or $\log p(x|\theta)$, so we can maximize it. 
...
tensor([0.])
```
In general, computing directly with probabilities quickly becomes very unstable, so for now on we'll turn our attention to computing $\log p(x|\theta)$ in the next part.